...
import cos.peerna.domain.room.repository.ConnectedUserRepository;
import cos.peerna.domain.room.repository.RoomRepository;
import cos.peerna.domain.room.repository.WaitingUserRepository;
...
public class RoomService {
        ...
        deferredResult.setResult(
                ResponseEntity.ok(
                        RoomResponseDto.builder()
                                ...
                                .historyId(history.getId())
                                .problem(history.getProblem())
                                .peer(new UserProfileData(userRepository.findById(peer.getId()).orElse(null)))
                                .build()));
    }
    ...
    public boolean findAlreadyExist(SessionUser user, DeferredResult<ResponseEntity<RoomResponseDto>> deferredResult, Integer player) {
        ...
        if (player == 2) {
            ...
            userProfileData = new UserProfileData(peer);
        }
